...
import { fetchCategories, fetchBrands, fetchSizes, addItem, fetchEditItem } from "../api";
...
interface SharedItemFormProps {
    isEditMode: boolean;
}

const ItemForm: React.FC<SharedItemFormProps> = ({ isEditMode }) => {
    ...
    const { data: editItem } = useQuery({
        ...
        queryFn: () => (id ? fetchEditItem(Number(id)) : Promise.resolve(null)),
        enabled: isEditMode && !!id,
        onSuccess: (data) => {
            if (data) {
                reset(data);
                setImages(data.pictures.map((src: string) => ({ src, file: null })));
            }
        },
    });
                ...
                <Grid item xs={12} md={6}>
                    ...
                    {[
                        ...
                        <Controller
                            ...
                            control={control}
                            defaultValue={editItem ? editItem[name as keyof FormValues] || "" : ""}
                            render={({ field }) =>
                                options ? (
